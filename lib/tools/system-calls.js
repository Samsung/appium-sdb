...
systemCallMethods.push = async function (file, dest) {
  try {
    await this.sdbExec(['push', file, dest]);
    return true;
  } catch (err) {
    log.warn(`Cannot push the file: '${err.message}'. Continuing`);
    return false;
  }
};

systemCallMethods.pull = async function (file, dest) {
  try {
    await this.sdbExec(['pull', file, dest]);
    return true;
  } catch (err) {
    log.warn(`Cannot pull the file: '${err.message}'. Continuing`);
    return false;
  }
};
