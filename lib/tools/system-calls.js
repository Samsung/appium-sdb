...
systemCallMethods.setWriteFileSystem = async function () {
  await this.shell(["mount -o remount,rw /"]);
};
...
systemCallMethods.installRpm = async function (rpm, path, opts = {}) {
  const fullPath = path ? path + "/" + rpm : rpm;
  await this.mkdir('rpmtemp');
  await this.push(fullPath, 'rpmtemp/');
  let result = await this.installRpmRemotePath(`rpmtemp/${rpm}`, opts);
  await this.rimraf('rpmtemp');
  return result;
};

systemCallMethods.installRpmRemotePath = async function (rpmPathOnDevice, opts = {}) {
  let stdout = await this.shell([`rpm -Uvh --force --nodeps ${rpmPathOnDevice}`], opts);
...
systemCallMethods.uninstallRpmRemote = async function (pkg, opts = {}) {
  await this.shell([`rpm -e ${pkg}`], opts);
};

systemCallMethods.isRpmInstalled = async function (pkg, opts = {}) {
  let installed = false;
  let stdout = await this.shell([`rpm -qa | grep ${pkg}`], opts);
...
systemCallMethods.checkServiceStatus = async function (service, opts = {}) {
  let status;
  let stdout = await this.shell([`systemctl status ${service}`], opts);

  if (stdout.indexOf('activating (start)') > -1) {
    status = true;
    log.debug(`${service} service is started`);
  } else if (stdout.indexOf('inactive (dead)') > -1) {
    status = false;
    log.debug(`${service} service is stoped`);
  } else {
    log.debug(`${service} service status : ${stdout}`);
  }
  return status;
};

systemCallMethods.startService = async function (service, opts = {}) {
  await this.shell([`systemctl start ${service}`], opts);

  let result = await this.checkServiceStatus(service, opts);
  if (result) {
    return true;
  } else {
    return false;
...
systemCallMethods.stopService = async function (service, opts = {}) {
  await this.shell([`systemctl stop ${service}`], opts);
